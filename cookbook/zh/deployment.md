...
本章节聚焦于如何在 AgentScope Runtime 上部署智能体。在完成概念与快速上手之后，部署是让实验性原型进入稳定运行的关键一步。我们将首先给出部署思路与整体流程，然后串联后续的服务、Agent App、进阶部署以及 React Agent 子章节，帮助你快速定位适合的路径。
...
2. **基础服务启动**：根据业务需要选择记忆、会话、沙箱等服务实现，并通过 `ServiceWithLifecycleManager` 接口进行生命周期管理。
3. **Agent App 定义**：基于 `agent_app` 模块编排智能体角色、工具与状态机逻辑，形成可部署的应用入口。
...
### Agent App（{doc}`zh/agent_app`）

`agent_app` 是将多个智能体、工具与上下文串联的应用形态。子章节会说明：
...
### 高级部署（{doc}`zh/advanced_deployment`）
...
### React Agent（{doc}`zh/react_agent`）

React Agent 章节介绍了如何在部署后提供可视化界面或 Web UI，让终端用户以交互式方式体验智能体。你将了解到：

- React 前端如何通过 WebSocket/REST 接入 Runtime。
- 如何在前端展示会话、工具调用结果与调试信息。
- 在部署层面如何处理静态资源构建、反向代理与鉴权。

如果应用需要对外提供 Web 体验或运营门户，该章节是重要参考。
...
2. 在 `Agent App` 中完成业务逻辑编排与本地验证。
3. 根据规模需求选择 `高级部署` 指南，完成生产化配置。
4. 若需要 Web 交互层，继续阅读 `React Agent` 章节并完成前端部署。
